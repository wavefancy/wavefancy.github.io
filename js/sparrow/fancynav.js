...
spUtils.$document.ready(() => {
  ...
  const isFancynavbarTop = $fancynavbar.hasClass(ClassName.FANCYNAVBAR_TOP);
  ...
  if(isFancynavbarTop){
    ...
    const $fancyNavbarTogglerIcon = $(Selector.FANCYNAVBAR_TOGGLER_ICON);
    const $fancyNavbarAddon = $(Selector.FANCYNAVBAR_ADDON);
    const anim = $fancynavbar.data(DATA_KEY.ZANIM_XS);
    const childAnim = { ...anim, delay: 0.4 };
    const addonAnim = { ...anim, delay: 0.5 };

    $fancynavbar.data(DATA_KEY.ZANIM_LG, anim);
    $fancyNavbarBrandImg.data(DATA_KEY.ZANIM_LG, childAnim);
    $fancyNavbarBrandImg.data(DATA_KEY.ZANIM_LG).from.y = -38;
    $fancyNavbarTogglerIcon.data(DATA_KEY.ZANIM_LG, childAnim);
    $fancyNavbarAddon.data(DATA_KEY.ZANIM_LG, addonAnim);
    $fancyNavbarAddon.data(DATA_KEY.ZANIM_LG).from.y = -30;
  }
  ...
  if ($fancynavbar.length) {
    ...
    $(Selector.FANCYNAVBAR_LINK).on(clickEvent, (e) => {
      const $this = $(e.target);
      ...
      if($this.hasClass(ClassName.FANCYNAV_LINK) 
        || $this.parent().hasClass(ClassName.FANCYNAV_LINK)){
      ...
      const changeFancyNavBG = () => {
        ...
        timer = window.setTimeout(function() {
          ...
          if (spUtils.$window.width() >= spUtils.breakpoints.lg) {
            ...
            $togglerbar.css({ backgroundColor: prevBgColor.replace('rgba', 'rgb').split(',').slice(0, 3).join() + ')' });
          } else {
